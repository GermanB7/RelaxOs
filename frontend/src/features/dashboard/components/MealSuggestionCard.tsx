import { Link } from 'react-router-dom'
import type { DashboardSummary } from '../../../shared/types/domain'

type Props = {
  mealSuggestion: DashboardSummary['mealSuggestion']
}

export function MealSuggestionCard({ mealSuggestion }: Props) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="text-base font-semibold text-slate-950">
        {mealSuggestion.title}
      </h2>
      <p className="mt-2 text-sm text-slate-600">{mealSuggestion.reason}</p>
      <Link
        to="/meals"
        className="mt-3 inline-flex rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100"
      >
        Open meals
      </Link>
    </section>
  )
}
